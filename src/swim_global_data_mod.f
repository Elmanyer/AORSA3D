...
!--------------------------------------------------------------------------
      ...
      INTEGER, PARAMETER ::
     1   rspec = SELECTED_REAL_KIND(p=15,r=307),
     1   ispec = SELECTED_int_KIND(r=9)
      integer, parameter :: swim_string_length = 64
...
!--------------------------------------------------------------------------
     ...
     1          TRIM(variable_name), ' in routine ', TRIM(calling_routin
     1            e)
            ELSE
                WRITE(SWIM_err_out_file,*) 'Allocation error in routine
     1           ', TRIM(calling_routine)
            END IF
     ...
     1           , TRIM(calling_routine)
        ...
        CASE DEFAULT
            WRITE(*,*) 'Unknown error kind in routine ',
     1       TRIM(calling_routine)
